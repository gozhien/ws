...
rm -rf badvpn-master key.pem cert.pem install.sh master.zip && apt -y remove cmake make zip unzip && /etc/init.d/dropbear restart && /etc/init.d/stunnel4 restart && clear
